...
void entry_setDisplay(void){   //inicijalizacija displeja
	LCD_RS=0; //saljemo komandu ne podatke na lcd
			  
	LCD_EN=1;  //lcd treba da procita data linije 
	LCD_D7=0;
	LCD_D6=0;
	LCD_D5=0;
	LCD_D4=0;
	...
	_nop_();	//cekamo jedan masinski ciklus jer koristimo samo gornja 4 bita
...
//	_nop_();
	...
	_nop_();		   //iz biblioteke intrins.h  --- ceka 1 masinski ciklus
...
void writeChar(uint8_t karakter) {
	...
	LCD_EN=1;					  
	LCD_D7=(karakter >> 7) & 0x01; //prvo donjih 4 bita zato siftuijemo za 7 da dodje na 0 mesto
...
void newLine(void) { //prebacuje uvek na 40 adresu, da bi se vratili na 00 mora clear		 				
